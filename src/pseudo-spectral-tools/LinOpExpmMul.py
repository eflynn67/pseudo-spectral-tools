...
def _expm_multiply_interval(
    ...
    if t_0 * A_1_norm == 0:
        ...
        norm_info_0 = LazyOperatorNormInfo(
            t_0 * A, A_1_norm=abs(t_0) * A_1_norm, ell=ell
        )
    ...
    if q <= s:
        if status_only:
            ...
            X, status = _expm_multiply_interval_core_0(
        ...
        if status_only:
            ...
            X, status = _expm_multiply_interval_core_1(A, X, h, mu, m_star, s, q, tol)
    elif q % s:
        if status_only:
            return 2
        else:
            X, status = _expm_multiply_interval_core_2(A, X, h, mu, m_star, s, q, tol)
    else:
